...
root.title("Calculator") #title shown on window
root.resizable(0, 0) #window is not reziable
...
#running the program into a loop continuously
#root.mainloop()

#creating a function for clicking
def buttonClick(num):
    temp = e.get() #temporary variable to store the current input in the screen
    e.delete(0, tk.END) #clearing screen from index 0 to end
    e.insert(0, temp + num) #inserting the current number input

def buttonClear(): #function for clearing the screen
    e.delete(0, tk.END)


def buttonGet(oper): #storing first input and printing +, -, /, *
    global num1, math #global variable num1 and math to use in function buttonEqual()
    num1 = e.get() #get first number
    math = oper #oper variable is the type of operation being performed.
    e.insert(tk.END, math)
    try:
        num1 = float(num1) #convering the number to float type
